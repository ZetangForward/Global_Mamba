class ModelConfig:
    ...
    def return_config(self, model_name_or_path, tokenizer_name_or_path, ckpt_path=None, conv1d_configs=None, *args, **kwargs):
        ...
        if "mamba" in model_name_or_path.lower(): # SSM model config
            ...
            if "130" in model_name_or_path.lower():  # 130M mamba
                return self.mamba_config(model_name_or_path='mamba-130m-hf', ckpt_path=ckpt_path, 
                                         ...
                                         *args, **kwargs) 
            elif "370" in model_name_or_path.lower():  # 370M mamba
                return self.mamba_config(model_name_or_path='mamba-370m-hf', ckpt_path=ckpt_path, 
                                        load_model_state_dict=ckpt_path is not None, conv1d_configs=conv1d_configs, 
                                         *args, **kwargs) 

            elif "790" in model_name_or_path.lower():
                return self.mamba_config('mamba-790m-hf', ckpt_path=ckpt_path, 
                                        load_model_state_dict=ckpt_path is not None, conv1d_configs=conv1d_configs, 
                                         *args, **kwargs)
                ...
                ssm_size = 16
                return self.tiny_mamba_config(model_name_or_path="mamba-130m-hf", tokenizer_name_or_path="mamba-130m-hf", 
                                             ckpt_path=ckpt_path, load_model_state_dict=ckpt_path is not None, 
                                             conv1d_configs=conv1d_configs, ssm_size=ssm_size, *args, **kwargs)
        ...
        elif "pythia-160m" in model_name_or_path.lower():
            return self.pythia_config(model_name_or_path="pythia-160m", 
                                       tokenizer_name_or_path="pythia-160m",
                                       ckpt_path=ckpt_path, load_model_state_dict=ckpt_path is not None)

        elif "pythia-410m" in model_name_or_path.lower():
            return self.pythia_410_config(model_name_or_path="pythia-410m", 
                                       tokenizer_name_or_path="pythia-410m",
                                       ckpt_path=ckpt_path, load_model_state_dict=ckpt_path is not None)

        elif "pythia-1b" in model_name_or_path.lower():
            return self.pythia_410_config(model_name_or_path="EleutherAI/pythia-1b", 
    ...
    def tiny_mamba_config(self, model_name_or_path=None, tokenizer_name_or_path=None, ckpt_path=None, load_model_state_dict = False,
                          conv1d_configs = None, use_custom_module = False, tiny_mamba_configs = None, ssm_size=16,  **kwargs):  
        import math
        tiny_mamba_configs = {"num_hidden_layers": 2, "hidden_size": 256, "intermediate_size": 512, 
                              "time_step_rank": 32, "ssm_state_size": ssm_size}
    ...
    def pythia_410_config(self, model_name_or_path,tokenizer_name_or_path ,ckpt_path, load_model_state_dict, use_custom_module=False):
        return {"model_name_or_path": model_name_or_path, "tokenizer_name_or_path": tokenizer_name_or_path,
                "load_model_state_dict": load_model_state_dict, "ckpt_path": ckpt_path, "use_custom_module": use_custom_module}
