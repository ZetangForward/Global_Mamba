                ...
                if str(self.layer_idx) in layer_list:
                    ...
                    for module in modules:
                        if module == "longconv" :
                            ...
                            self.concat_net = FusionNetwork(config.intermediate_size)
                            if layer_idx==0: log_c(f"concat, using FusionNetwork")
                        elif module == "dtlinear":
                            self.dt_linear = DT_linear(config.intermediate_size, self.time_step_rank)
                            if layer_idx==0: log_c(f"dtlinear, using DT_linear")
                        elif module == "shallow":
...
######################## Custom module sequence transformation   ##################################
        ...
        if self.module and "longconv" in self.module_type and self.module_type!="shortlongconv":
            ...
            module_output = self.module(module_input).to(hidden_states.dtype)
        
        if "attn" in self.module_type:
            module_input = hidden_states.clone()
            module_input = module_input.to(hidden_states.dtype)
            module_output = self.module(module_input).to(hidden_states.dtype)
...
##########################################################################################
        ...
        if "+" in self.module_type and module_output is not None:
            module_act = self.act(module_output)   # todo  flash longconv 去掉self.act
        ...
        # if input_states.shape[1]==65 and self.layer_idx==15:
